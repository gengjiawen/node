...
const { observerCounts: counts } = internalBinding('performance');
...

{
  ...
  [1, null, undefined].forEach((input) => {
    common.expectsError(() => observer.observe(input), {
...
// Test Non-Buffered
{
  const observer = new PerformanceObserver(common.mustCall(callback, 3));
...
// Test Buffered
{
  const observer = new PerformanceObserver(common.mustCall(callback, 1));
  assert.strictEqual(counts[NODE_PERFORMANCE_ENTRY_TYPE_MARK], 0);
