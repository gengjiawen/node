...
const hexBufFill = Buffer.allocUnsafe(2)
  .fill(0)
  .fill('0102', 'hex');
...
[
  ...
  common.expectsError(() => buf1.fill(...args), { code: 'ERR_OUT_OF_RANGE' });
});

common.expectsError(() => buf1.fill('a', 0, buf1.length, 'node rocks!'), {
  code: 'ERR_UNKNOWN_ENCODING',
  type: TypeError,
  message: 'Unknown encoding: node rocks!'
});

[['a', 0, 0, NaN], ['a', 0, 0, false]].forEach((args) => {
  common.expectsError(() => buf1.fill(...args), {
    code: 'ERR_INVALID_ARG_TYPE',
    message:
      'The "encoding" argument must be of type ' +
      `string. Received type ${args[3] === null ? 'null' : typeof args[3]}`
  });
});

common.expectsError(() => buf1.fill('a', 0, 0, 'foo'), {
  code: 'ERR_UNKNOWN_ENCODING',
  type: TypeError,
...
common.expectsError(() => Buffer.allocUnsafe(8).fill('a', -1), {
  code: 'ERR_OUT_OF_RANGE'
});
common.expectsError(() => Buffer.allocUnsafe(8).fill('a', 0, 9), {
  code: 'ERR_OUT_OF_RANGE'
});
