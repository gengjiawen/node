...
assert.throws(
  () => fs.read(fd, expected.length, 0, 'utf-8', common.mustNotCall()),
  {
    ...
    message:
      'The "buffer" argument must be one of type Buffer, TypedArray, ' +
...
assert.throws(
  ...
  },
  {
    ...
    message:
      'The value of "offset" is out of range. It must be >= 0 && <= 4. ' +
      'Received -1'
  }
);
...
assert.throws(() => fs.readSync(fd, expected.length, 0, 'utf-8'), {
  code: 'ERR_INVALID_ARG_TYPE',
  name: 'TypeError [ERR_INVALID_ARG_TYPE]',
  message:
    'The "buffer" argument must be one of type Buffer, TypedArray, ' +
