...
// Test 8 bit
{
  ...
  ['', '0', null, {}, [], () => {}, true, false].forEach((off) => {
    assert.throws(() => buffer.writeInt8(23, off), {
...
// Test 16 bit
{
  ...
  ['writeInt16BE', 'writeInt16LE'].forEach((fn) => {
    ...
    ['', '0', null, {}, [], () => {}, true, false].forEach((off) => {
      assert.throws(() => buffer[fn](23, off), {
        code: 'ERR_INVALID_ARG_TYPE'
      });
    });
...
// Test 48 bit
{
  ...
  assert.ok(
    buffer.equals(new Uint8Array([0x12, 0x34, 0x56, 0x78, 0x90, 0xab]))
  );

  buffer.writeIntLE(value, 0, 6);
  assert.ok(
    buffer.equals(new Uint8Array([0xab, 0x90, 0x78, 0x56, 0x34, 0x12]))
  );
}
