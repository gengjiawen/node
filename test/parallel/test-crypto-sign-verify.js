...

{
  ...
  assert(
    instance instanceof Sign,
    'Sign is expected to return a new ' + 'instance when called without `new`'
  );
}

{
  ...
  assert(
    instance instanceof Verify,
    'Verify is expected to return a new ' + 'instance when called without `new`'
  );
}
    ...
    signSaltLengths.forEach((signSaltLength) => {
      if (signSaltLength > max) {
        ...
        verifySaltLengths.forEach((verifySaltLength) => {
          ...
          const saltLengthCorrect =
            getEffectiveSaltLength(signSaltLength) ===
...
{
  [null, undefined, NaN, 'boom', {}, [], true, false].forEach(
    (invalidValue) => {
      common.expectsError(
        () => {
          crypto
...
// RSA-PSS Sign test by verifying with 'openssl dgst -verify'
{
  ...
  const cmd = `"${
    common.opensslCli
  }" dgst -sha256 -verify "${pubfile}" -signature "${sigfile}" -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:-2 "${msgfile}"`;
...

{
  ...
  [1, [], {}, undefined, null, true, Infinity].forEach((input) => {
    ...
    errObj.message =
      'The "data" argument must be one of type string, ' +
  ...
  [Uint8Array, Uint16Array, Uint32Array, Float32Array, Float64Array].forEach(
    (clazz) => {
  ...
  [1, {}, [], Infinity].forEach((input) => {
    ...
    errObj.message =
      'The "signature" argument must be one of type string, ' +
      `Buffer, TypedArray, or DataView. Received type ${type}`;
