...
function testClients(getSocketOpt, getConnectOpt, getConnectCb) {
  ...
  return [
    ...
    net.connect(cloneOptions(1)).on('connect', getConnectCb(1)),
    net.createConnection(cloneOptions(2), getConnectCb(2)),
    net.createConnection(cloneOptions(3)).on('connect', getConnectCb(3)),
    new net.Socket(getSocketOpt(4)).connect(
      getConnectOpt(4),
      getConnectCb(4)
    ),
    new net.Socket(getSocketOpt(5))
      .connect(getConnectOpt(5))
...
// Test Pipe fd is wrapped correctly
{
  ...
  const server = net
    ...
    .on(
      'connection',
      forAllClients(function serverOnConnection(socket) {
        let clientFd;
        socket.on(
          'data',
        ...
        socket.on(
          ...
          common.mustCall(function() {
            counter++;
            console.error(
              `[Pipe]Received end from fd ${clientFd}, total ${counter}`
            );
    ...
    .listen(
      { path: serverPath },
      common.mustCall(function serverOnListen() {
        ...
        const getConnectCb = (index) =>
          common.mustCall(function clientOnConnect() {
