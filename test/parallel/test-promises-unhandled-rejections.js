...
asyncTest(
  'Promise rejected after setTimeout(,1) should trigger' +
    ' unhandled rejection',
  function(done) {
    const e = new Error();
    onUnhandledSucceed(done, function(reason, promise) {
...
asyncTest(
  'Catching a promise rejection after setImmediate is not' +
    ' soon enough to stop unhandledRejection',
  function(done) {
    const e = new Error();
    onUnhandledSucceed(done, function(reason, promise) {
      assert.strictEqual(reason, e);
...
asyncTest(
  'When re-throwing new errors in a promise catch, only the' +
    ' re-thrown error should hit unhandledRejection',
  function(done) {
    const e = new Error();
    const e2 = new Error();
    onUnhandledSucceed(done, function(reason, promise) {
...
asyncTest(
  'When re-throwing new errors in a promise catch, only the ' +
    ...
    ' rejected async with setTimeout(,1)',
  function(done) {
    const e = new Error();
    const e2 = new Error();
    onUnhandledSucceed(done, function(reason, promise) {
...
asyncTest(
  'Rejected promise inside unhandledRejection allows nextTick loop' +
    ' to proceed first',
  function(done) {
    clean();
    Promise.reject(0);
