...
vals.forEach((v) => {
  common.expectsError(() => http.request({ hostname: v }), {
    code: 'ERR_INVALID_ARG_TYPE',
    type: TypeError,
    message:
      'The "options.hostname" property must be one of ' +
      'type string, undefined, or null. ' +
      `Received type ${typeof v}`
  });

  common.expectsError(() => http.request({ host: v }), {
    code: 'ERR_INVALID_ARG_TYPE',
    type: TypeError,
    message:
      'The "options.host" property must be one of ' +
      'type string, undefined, or null. ' +
      `Received type ${typeof v}`
  });
});
