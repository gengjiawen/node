...

{
  ...
  assert(
    ...
    'Hmac is expected to return a new instance' + ' when called without `new`'
  );
}
...
common.expectsError(() => crypto.createHmac('sha1', null), {
  code: 'ERR_INVALID_ARG_TYPE',
  type: TypeError,
  message:
    'The "key" argument must be one of type string, TypedArray, or ' +
...
const wikipedia = [
  {
    ...
    hmac: {
      ...
      sha256:
        'f7bc83f430538424b13298e6aa6fb143ef4d59a14946175997479dbc' + '2d1a3cd8'
    }
  },
  {
    key: 'key',
    ...
    hmac: {
      ...
      sha256:
        '5d5d139563c95b5967b9bd9a8c9b233a9dedb45072794cd232dc1b74' + '832607d0'
    }
  },
  {
    key: '',
    ...
    hmac: {
      ...
      sha256:
        'fb011e6154a19b9a4c767373c305275a5a69e8b68b0b4c9200c383dc' + 'ed19a416'
    }
  },
  {
    key: '',
    ...
    hmac: {
      ...
      sha256:
        'b613679a0814d9ec772f95d778c35fc5ff1697c493715653c6c71214' + '4292c5ad'
    }
  }
];
...
for (let i = 0, l = wikipedia.length; i < l; i++) {
  for (const hash in wikipedia[i].hmac) {
    ...
    if (common.hasFipsCrypto && hash === 'md5') continue;
    const expected = wikipedia[i].hmac[hash];
    const actual = crypto
      .createHmac(hash, wikipedia[i].key)
...
if (!common.hasFipsCrypto) {
  for (let i = 0, l = rfc2202_md5.length; i < l; i++) {
    const actual = crypto
      .createHmac('md5', rfc2202_md5[i].key)
...
for (let i = 0, l = rfc2202_sha1.length; i < l; i++) {
  const actual = crypto
    .createHmac('sha1', rfc2202_sha1[i].key)
...
common.expectsError(() => crypto.createHmac('sha256', 'w00t').digest('ucs2'), {
