...
const { UV_EBADF, UV_EINVAL, UV_ENOTSOCK } = internalBinding('uv');
...

{
  ...
  const errorObj = {
    ...
    type: SystemError,
    message:
      'Could not get or set buffer size: uv_recv_buffer_size ' +
...

{
  ...
  const errorObj = {
    ...
    type: SystemError,
    message:
      'Could not get or set buffer size: uv_send_buffer_size ' +
      'returned EINVAL (invalid argument)',
    info
  };
  const socket = dgram.createSocket('udp4');
