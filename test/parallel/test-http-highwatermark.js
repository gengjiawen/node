  ...
  .createServer(function(req, res) {
    ...
    if (id === 1) {
      ...
      req.connection.once(
        ...
        common.mustCall(() => {
          assert(
            req.connection._paused,
            '_paused must be true because it exceeds' +
      ...
      req.connection.parser.resume = common.mustCall((...args) => {
        ...
        assert(
          !paused,
          '_paused must be false because it become false by ' +
    ...
    assert(
      !res.write(body),
      'res.write must return false because it will ' +
