...
server.on(
  ...
  common.mustCall((session) => {
    ...
    [0, -1, 1.1, 0xffffffff + 1, Infinity, -Infinity].forEach((input) => {
      assert.throws(() => session.altsvc('h2=":8000"', input), {
        code: 'ERR_OUT_OF_RANGE',
        name: 'RangeError [ERR_OUT_OF_RANGE]',
        message:
          'The value of "originOrStream" is out of ' +
    ...
    [0, {}, [], null, Infinity].forEach((input) => {
      assert.throws(() => session.altsvc(input), {
    ...
    [{}, [], true].forEach((input) => {
      assert.throws(() => session.altsvc('clear', input), {
        code: 'ERR_INVALID_ARG_TYPE',
        name: 'TypeError [ERR_INVALID_ARG_TYPE]'
      });
    });

    ['abc:', new URL('abc:'), { origin: 'null' }, { origin: '' }].forEach(
      (input) => {
        assert.throws(() => session.altsvc('h2=":8000', input), {
...
server.listen(
  ...
  common.mustCall(() => {
    ...
    client.on(
      'altsvc',
      common.mustCall((alt, origin, stream) => {
