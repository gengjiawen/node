...
server.on(
  ...
  common.mustCall((stream) => {
    assert(
      stream.session.ping(
        common.mustCall((err, duration, ret) => {
      ...
      common.mustCall(() => {
        ...
        // Should throw if payload is not of type ArrayBufferView
        {
          [1, true, {}, []].forEach((payload) =>
            common.expectsError(() => client.ping(payload), {
              type: TypeError,
              code: 'ERR_INVALID_ARG_TYPE',
              message:
                'The "payload" argument must be one of type Buffer, ' +
        ...
        // Should throw if payload length is not 8
        {
          ...
          [shortPayload, longPayload].forEach((payloadWithInvalidLength) =>
            common.expectsError(() => client.ping(payloadWithInvalidLength), {
              type: RangeError,
        ...
        // Should throw error is callback is not of type function
        {
          ...
          [1, true, {}, []].forEach((invalidCallback) =>
            common.expectsError(() => client.ping(payload, invalidCallback), {
              type: TypeError,
