...
(function() {
    ...
    parsed_idls.forEach(
      function(parsed_idl) {
        ...
        if (parsed_idl['inheritance']) {
          ...
          if (
            parsed_idl.name in this['inheritance'] &&
  ...
  IdlArray.prototype.recursively_get_implements = function(interface_name) {
    ...
    for (var i = 0; i < this['implements'][interface_name].length; i++) {
      ...
      if (ret.indexOf(ret[i]) != ret.lastIndexOf(ret[i])) {
        throw new IdlHarnessError(
          'Circular implements statements involving ' + ret[i]
        );
      }
    }
    return ret;
  ...
  IdlArray.prototype.recursively_get_includes = function(interface_name) {
    ...
    for (var i = 0; i < this['includes'][interface_name].length; i++) {
      ...
      if (ret.indexOf(ret[i]) != ret.lastIndexOf(ret[i])) {
        throw new IdlHarnessError(
          'Circular includes statements involving ' + ret[i]
        );
      }
    }
    return ret;
      ...
      default:
        ...
        if (thing instanceof IdlInterface) {
          ...
          while (thing) {
            ...
            var mixins =
              this.implements[thing.name] || this.includes[thing.name];
            if (mixins) {
          ...
          test(
            function() {
              ...
              assert_true(
                ...
                `Original ${parsed_idl.name} definition should have type ${
                  parsed_idl.type
                }`
              );
            }.bind(this),
            `Partial ${parsed_idl.type} ${partialTestName}: original ${
              parsed_idl.type
            } defined`
          );
        }
          ...
          if (exposureAttr) {
            if (!parsed_idl.untested) {
              test(
                function() {
                  ...
                  partialExposure.forEach((name) => {
                    if (!memberExposure || !memberExposure.has(name)) {
                      throw new IdlHarnessError(
                        `Partial ${parsed_idl.name} ${
                          parsed_idl.type
                        } is exposed to '${name}', the original ${
                          parsed_idl.type
                        } is not.`
                      );
                    }
                ...
                `Partial ${
                  parsed_idl.type
                } ${partialTestName}: valid exposure set`
              );
            }
  ...
  IdlArray.prototype.assert_type_is = function(value, type) {
    ...
    if (this.members[type] instanceof IdlInterface) {
      ...
      throw new IdlHarnessError(
        'Type ' + type + " isn't an interface or dictionary"
      );
    }
  };
  ...
  IdlInterface.prototype.get_interface_object = function() {
    if (this.has_extended_attribute('NoInterfaceObject')) {
      throw new IdlHarnessError(
        this.name + ' has no interface object due to NoInterfaceObject'
      );
    }
  ...
  IdlInterface.prototype.traverse_inherited_and_consequential_interfaces = function(
    callback
  ) {
  ...
  function _traverse_inherited_and_consequential_interfaces(stack, callback) {
    ...
    if (mixins) {
      mixins.forEach(function(id) {
        ...
        if (!mixin) {
          throw new Error(
            'Interface ' + id + ' not found (implemented by ' + I.name + ')'
          );
        }
    ...
    subsetTestByKey(
      ...
      function() {
        ...
        if (this.base) {
          ...
          if (
            !inherited_interface.has_extended_attribute('NoInterfaceObject')
          ) {
  ...
  IdlInterface.prototype.test_to_json_operation = function(
    ...
    if (member.has_extended_attribute('Default')) {
      subsetTestByKey(
        ...
        'Test default toJSON operation of ' + instanceName
      );
    } else {
      subsetTestByKey(
        this.name,
        ...
        'Test toJSON operation of ' + instanceName
      );
    }
  };
  ...
  IdlInterface.prototype.test_member_iterable = function(member) {
    ...
    subsetTestByKey(
      ...
      function() {
        ...
        assert_equals(
          this.get_interface_object().prototype[Symbol.iterator].name,
          isPairIterator ? 'entries' : 'values',
          '@@iterator function does not have the right name'
        );
      }.bind(this),
    ...
    if (isPairIterator) {
      subsetTestByKey(
        ...
        function() {
          assert_equals(
            this.get_interface_object().prototype[Symbol.iterator],
            this.get_interface_object().prototype['entries'],
            'entries method is not the same as @@iterator'
          );
        }.bind(this),
      ...
      subsetTestByKey(
        ...
        test,
        function() {
          ['entries', 'keys', 'values', 'forEach', Symbol.Iterator].forEach(
            function(property) {
              assert_equals(
                this.get_interface_object().prototype[property],
                Array.prototype[property],
                property + ' function is not the same as Array one'
              );
            }.bind(this)
          );
        }.bind(this),
    ...
    for (var i = 0; i < this.members.length; i++) {
      ...
      if (member.type == 'attribute' && member.isUnforgeable) {
        ...
        if (member.type == 'operation') {
          described_name +=
            '(' +
            member.arguments.map((arg) => arg.idlType.idlType).join(', ') +
            ')';
        }
        subsetTestByKey(
          this.name,
  ...
  IdlNamespace.prototype.test = function() {
    ...
    for (const v of Object.values(this.members)) {
      switch (v.type) {
        ...
        default:
          throw 'Invalid namespace member ' +
            v.name +
            ': ' +
            v.type +
            ' not supported';
