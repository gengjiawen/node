(function() {
  ...
  window.test_driver = {
    ...
    bless: function(intent, action) {
      ...
      button.innerHTML =
        'This test requires user interaction.<br />' +
        'Please click here to allow ' +
        intent +
        '.';
    ...
    click: function(element) {
      ...
      if (!window.document.contains(element)) {
        return Promise.reject(
          new Error('element in different document or shadow tree')
        );
      }
      ...
      var pointerInteractablePaintTree = getPointerInteractablePaintTree(
        element
      );
      if (
        pointerInteractablePaintTree.length === 0 ||
    ...
    send_keys: function(element, keys) {
      if (window.top !== window) {
        return Promise.reject(
          new Error('can only send keys in top-level window')
        );
      }
