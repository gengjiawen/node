...
(() => {
  function write(ast, opt = {}) {
    ...
    function make_ext_at(it) {
      ...
      let ret = it.name;
      if (it.rhs) {
        if (it.rhs.type === 'identifier-list')
          ret += `=(${it.rhs.value.join(',')})`;
        else ret += `=${it.rhs.value}`;
      }
      if (it.arguments)
        ret += `(${
          it.arguments.length ? it.arguments.map(argument).join(',') : ''
        })`;
    ...
    function const_(it) {
      ...
      return `${ret}const ${type(it.idlType)}${it.nullable ? '?' : ''} ${
        it.name
      } = ${const_value(it.value)};`;
    }
    function typedef(it) {
      let ret = extended_attributes(it.extAttrs);
    ...
    function callback(it) {
      const ret = extended_attributes(it.extAttrs);
      return `${ret}callback ${it.name} = ${type(
        it.idlType
      )}(${it.arguments.map(argument).join(',')});`;
    }
    function enum_(it) {
      let ret = extended_attributes(it.extAttrs);
      ret += `enum ${it.name} {`;
    ...
    function maplike(it) {
      return `${it.readonly ? 'readonly ' : ''}maplike<${it.idlType
        .map(type)
        .join(', ')}>;`;
    }
    function setlike(it) {
      return `${it.readonly ? 'readonly ' : ''}setlike<${type(
        it.idlType[0]
      )}>;`;
    }
