...
ModuleWrap::ModuleWrap(Environment* env,
                       ...
                       Local<Module> module,
                       Local<String> url)
    : BaseObject(env, object), id_(env->get_next_module_id()) {
...
void ModuleWrap::New(const FunctionCallbackInfo<Value>& args) {
  ...
  host_defined_options->Set(
      isolate, HostDefinedOptions::kID, Number::New(isolate, obj->id()));
...
void ModuleWrap::Resolve(const FunctionCallbackInfo<Value>& args) {
  ...
  if (!obj.IsEmpty()) args.GetReturnValue().Set(obj.ToLocalChecked());
}
...
static MaybeLocal<Promise> ImportModuleDynamically(
  ...
  if (import_callback
          ->Call(
              context, v8::Undefined(iso), arraysize(import_args), import_args)
          .ToLocal(&result)) {
...
void ModuleWrap::Initialize(Local<Object> target,
  ...
  target
      ->Set(env->context(),
            FIXED_ONE_BYTE_STRING(isolate, "ModuleWrap"),
